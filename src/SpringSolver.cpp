...
void SpringSolver::accumulateForces()
{
	...
	F.segment<3>(283*3) = Eigen::Vector3f::Zero();
	F.segment<3>(268*3) = Eigen::Vector3f::Zero();
...
void SpringSolver::implicitSolver()
{
	...
	dv.segment<3>(283 * 3) = Eigen::Vector3f::Zero();
	dv.segment<3>(268 * 3) = Eigen::Vector3f::Zero();
	currVel += dv;
	currPos += dt * currVel;
	for (int i = 0; i < n; i++)
	{
